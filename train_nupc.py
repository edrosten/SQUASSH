...
def _main()->None:
    ...
    data = [
        ('RESI', [t.to(device.device).half() for t in resi_data.load_3d()]),
        ('Bates', [t.to(device.device).half() for l in mark_bates_data.load_3d_list() for t in l])
    ]

    for name, nupc3d in data:
        mult = 20
        scatter = 0.01
        ...
        train.retrain(fast, dataset_initial, params_initial, f'{name}/phase_0')
        ...
        parameterisation.max_stretch_factor_expand = torch.tensor(1.5)
        
        torch.compiler.reset() # Otherwise it crashes on torch 2.7
        fast = cast(network.GeneralPredictReconstruction, torch.compile(net))
        train.retrain(fast, dataset_refine, params_refine, f'{name}/phase_1')
